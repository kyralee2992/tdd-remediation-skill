...
async function tddAuditPlugin(fastify, opts) {
  ...
  // Security headers are applied by the root-level onSend hook in buildApp()
  // so that they cover ALL responses including Fastify's built-in 404 handler.
...
function buildApp(cfg, overrides = {}) {
  ...
  // Root-level hook: applies to ALL responses including 404s from Fastify's default handler.
